...
public interface QualificationRepository extends JpaRepository<Qualification, UUID>{
	...
	@Query(value = "SELECT q FROM Qualification q JOIN q.processOwner po JOIN e.initiatedBy ib WHERE (po.id = :userId OR ib.id = :userId) AND q.status = :status")
	List<Qualification> findRequestsForOwnerOrInitiatorByStatus(@Param("userId") UUID userId, @Param("status") String status);
	
	@Query(value = "SELECT q FROM Qualification q JOIN q.processOwner po JOIN e.initiatedBy ib WHERE po.id = :userId OR ib.id = :userId")
